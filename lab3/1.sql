...
SELECT department, COUNT(*) AS employess_code FROM employees GROUP BY department;

SELECT department, COUNT(*) AS employess_code FROM employees GROUP BY department HAVING COUNT(*) > 2;

SELECT name FROM departments ORDER BY budget DESC OFFSET 1 LIMIT 1;

SELECT employees.name, employees.lastname FROM employees WHERE department = (
...
SELECT code, name, budget * 0.9 FROM departments ORDER BY budget  LIMIT 1;
